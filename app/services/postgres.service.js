module.exports = (logger, pool) => {
  return {
    ...
    async findByShortUrl(shortUrl) {
      try {
        return await this.query({
          text: 'SELECT * FROM "shortenedLinks" WHERE "id" = $1;',
          values: [shortUrl],
        });
      } catch (err) {
        logger.logError(err, {
    ...
    async findByLongUrl(longUrl) {
      try {
        return await this.query({
          text: 'SELECT * FROM "shortenedLinks" WHERE "longUrl" = $1;',
          values: [longUrl],
        });
      } catch (err) {
        logger.logError(err, {
    ...
    async deleteStaleData(shortUrl) {
      await this.query({
        text: 'DELETE FROM "shortenedLinks" WHERE "id" = $1;',
        values: [shortUrl],
      });
      await this.query({
        text: 'DELETE FROM "shortUrlExpiry" WHERE "id" = $1;',
        values: [shortUrl],
      });
    },
    async isTTLExpired(shortUrl) {
      try {
        const result = await this.query({
          text: 'SELECT * FROM "shortUrlExpiry" WHERE "id" = $1;',
          values: [shortUrl],
        });
